...
\begin{itemize}
\item $e_1$이 값이 아니면 진행 $e_1 \to e_1'$하고, 이는 곧 프로그램 실행 $\to$의 정의에 의해 $e_1 \texttt{+} e_2 \to e_1' \texttt{+} e_2$와 같다.
\item $e_1$이 값이고 $e_2$가 값이 아니면 진행 $e_2 \to e_2'$하고, 이는 곧 프로그램 실행 $\to$의 정의에 의해 $e_1 \texttt{+} e_2 \to e_1 \texttt{+} e_2'$와 같다.
...
\item $e_1$, $e_2$가 모두 값인 경우,  진행 $e_1 \texttt{+} e_2 \to (n_1 + n_2)$ 하고 $\Gamma \vdash (n_1 + n_2) : \iota$
\end{itemize}
\textbf{Case 5.} $e = e_1 e_2$인 경우는 `slide 16.pdf'에 나와있다.
